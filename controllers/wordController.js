...
exports.words = function (req, res, next) {
  ...
  Word.find(
    { user: '5b21fd3946969f00144f2622', story: '5b2602d8b2d1122bbce72078' },
    { title: 1, _id: 0 }
  ).exec(function (err, list_words) {
    if (err) {
      return next(err);
...
exports.word_popup = function (req, res, next) {
  async.parallel(
    ...
    function (err, results) {
      ...
      if (
        typeof results.word.oxford_word != 'undefined' &&
        results.word.oxford_word != null
      ) {
        if (
          typeof results.word.oxford_word.word != 'undefined' &&
          results.word.oxford_word.word != null
        ) {
          results.word.title = results.word.oxford_word.word;
        }
      }
      res.render('word_popup', {
        word: results.word,
...
exports.wrdSave = function (req, res, next) {
  Word.update(
    { _id: req.body.id },
    {
      content: req.body.content,
...
exports.word_create_post = [
  ...
  (req, res, next) => {
    if (req.body.story_user != req.session.userId) {
      ...
      Word.find({ story: req.body.story_id, title: req.body.title }).exec(
        function (err, results) {
          //console.log(results);
          if (results.length > 0) {
            // No results.
            ...
            if (book_id.match(/^[0-9a-fA-F]{24}$/)) {
              word = new Word({
                ...
                oxford_word:
                  req.body.oxfordWord_id == '' ? null : req.body.oxfordWord_id,
...
exports.word_update_post = function (req, res, next) {
  if (req.body.story_user != req.session.userId) {
    ...
    Word.findById({ _id: req.body.id }).exec(function (err, theWord) {
      //console.log(theWord);
      if (theWord != null) {
        Word.update(
          { _id: req.body.id },
            ...
            .exec(function (err, list_words) {
              ...
              for (let i = 0; i < list_words.length; i++) {
                ...
                if (list_words[i].create_date != null) {
                  list_words[i].create_date = moment(
                    list_words[i].create_date
                  ).format('YYYY-MM-DD');
                }
                if (
                  list_words[i].story != null &&
                  list_words[i].story.title != null
                ) {
                  list_words[i].story.title = entities.decodeHTML(
                    list_words[i].story.title
                  );
                }
                if (
                  list_words[i].story_title != null &&
                  list_words[i].story_title != null
                ) {
                  list_words[i].story_title = entities.decodeHTML(
                    list_words[i].story_title
                  );
                }
                if (
                  list_words[i].book_title != null &&
                  list_words[i].book_title != null
                ) {
                  list_words[i].book_title = entities.decodeHTML(
                    list_words[i].book_title
                  );
                }
                if (
                  list_words[i].oxford_word != null &&
                  list_words[i].oxford_word.word != null
                ) {
...
exports.word_board_list = function (req, res, next) {
  ...
  if (typeof req.body.stle != 'undefined' && req.body.stle != '') {
          ...
          if (list_words[i].create_date != null) {
            for (let j = 0; j < list_words[i].create_date.length; j++) {
              if (list_words[i].create_date[j].user == req.session.userId) {
                list_words[i].cdate = moment(
                  list_words[i].create_date[j].c_date
                ).format('YYYY-MM-DD');
              }
            }
    ...
    req.body.skill != '' &&
    (typeof req.body.importance == 'undefined' || req.body.importance == '')
  ) {
    ...
    Word.find({ user: { $in: [req.session.userId] } })
      .count()
      .exec(function (err, count) {
        ct = count;
      });
