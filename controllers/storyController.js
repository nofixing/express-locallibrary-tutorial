...
var tooltip_naver_url =
  'http://tooltip.dic.naver.com/tooltip.nhn?languageCode=4&nlp=false&wordString=';
...
exports.download_get = function (req, res, next) {
  ...
  Story.findById(req.params.id).exec(function (err, theStory) {
    ...
    fs.readFile(
      rtd + '/public/download_template.html',
      'utf8',
      function (err, data) {
        if (err) {
          return next(err);
...
exports.story_list = function (req, res, next) {
  ...
  Story.find({ $and: [{ user: req.session.userId }, { book: null }] })
    .collation({ locale: 'en' })
    .sort({ create_date: -1 })
...
exports.story_open_list = function (req, res, next) {
  ...
  if (typeof req.body.stle != 'undefined' && req.body.stle != '') {
    Story.find({
      open: 'Y',
      $or: [
        { title: { $regex: '.*' + req.body.stle + '.*' } },
        { btitle: { $regex: '.*' + req.body.stle + '.*' } },
      ],
    })
      .count()
...
exports.story_list_ajax = function (req, res, next) {
  Story.find({ book: req.body.book })
    ...
    .exec(function (err, list_stories) {
      ...
      for (let i = 0; i < list_stories.length; i++) {
        if (
          typeof list_stories[i].chapter == 'undefined' ||
  ...
  async.parallel(
    {
      ...
      comments: function (callback) {
        ...
        Comment.find({ story: req.params.id })
          .populate('user')
          .sort({ create_date: 1 })
          .exec(callback);
      },
      words: function (callback) {
        ...
        /*
            ...
            */
        Word.find({ story: req.params.id })
          .populate('oxford_word', 'word')
          .collation({ locale: 'en' })
          .sort({ index_of: 1, create_date: 1 })
          .exec(callback);
      },
      memo: function (callback) {
        ...
        /*
            ...
            */
        Memo.find({ story: req.params.id }).exec(callback);
      },
      chatGPT: function (callback) {
        ChatGPT.find({ story: req.params.id }).exec(callback);
      },
      bookMark: function (callback) {
        BookMark.find({ user: req.session.userId, story: req.params.id }).exec(
          callback
        );
      },
      files: function (callback) {
        File.find({ user: req.session.userId, story: req.params.id }).exec(
          callback
        );
      },
      /*
    ...
    function (err, results) {
      ...
      if (results.story.user != req.session.userId) {
        if (
          typeof results.story.open == 'undefined' ||
          results.story.open != 'Y'
        ) {
      ...
      History.find({ user: req.session.userId, story: req.params.id }).exec(
        function (err, theHistory) {
          if (err) {
            console.log(err);
            return next(err);
          }
      ...
      for (let i = 0; i < results.words.length; i++) {
        if (
          results.words[i].oxford_word != null &&
          results.words[i].oxford_word.word != null
        ) {
          results.words[i].orgTitle = results.words[i].oxford_word.word.replace(
            /_/g,
            ' '
          );
        } else {
      ...
      if (req.body.tlp == 'y') {
        ...
        for (let i = 0; i < results.words.length; i++) {
          if (
            typeof results.words[i].image_address != 'undefined' &&
            typeof results.words[i].content != 'undefined'
          ) {
            ...
            var ttl = 'ttwrd' + i;
            var add_cnt = '';
            if (typeof results.words[i].add_content != 'undefined')
              add_cnt = results.words[i].add_content;
            sTag +=
              '<div id="' +
              ttl +
              '" style="line-height: 1.2;">' +
              ssc +
              results.words[i].content +
              add_cnt +
              '</div>\n';
            highlightHtml =
              '<span class="tltp" data-tooltip-content="#' +
              ttl +
              '">$1</span>';
          ...
          txt = txt.replace(
            new RegExp('(' + '\\b' + results.words[i].title + '\\b' + ')', 'i'),
            highlightHtml
          );
        }
      ...
      var chatGPT = '';
      var chatGPT_id = '';
      if (results.chatGPT.length > 0) {
        chatGPT = entities.decodeHTML(results.chatGPT[0].content);
        chatGPT_id = results.chatGPT[0]._id;
      }
      ...
      for (let i = 0; i < results.story.genre.length; i++) {
        results.story.genre[i].name = entities.decodeHTML(
          results.story.genre[i].name
        );
      }
  ...
  async.parallel(
    {
      books: function (callback) {
        Book.find({ user: req.session.userId }, 'title ').exec(callback);
      },
      ...
      async.parallel(
        {
          ...
          genres: function (callback) {
            Genre.find({ user: req.session.userId }, 'name ').exec(callback);
          },
        },
        function (err, results) {
          if (err) {
          ...
          for (let i = 0; i < results.genres.length; i++) {
            results.genres[i].name = entities.decodeHTML(
  ...
  async.parallel(
    ...
    function (err, results) {
      ...
      Story.findByIdAndRemove(req.body.id, function deleteStory(err) {
        ...
        History.find({ user: req.session.userId, story: req.body.id })
          .remove()
          .exec();
        res.redirect(aul);
      });
  ...
  async.parallel(
    ...
    function (err, results) {
      ...
      for (
        ...
        results.genres[all_g_iter].name = entities.decodeHTML(
          results.genres[all_g_iter].name
        );
        for (
          var story_g_iter = 0;
          story_g_iter < results.story.genre.length;
          story_g_iter++
        ) {
          if (
            results.genres[all_g_iter]._id.toString() ==
            results.story.genre[story_g_iter]._id.toString()
          ) {
...
exports.story_oxford = function (req, res, next) {
  ...
  lookup.then(
    ...
    function (err) {
      console.log(
        'req.query.word:' + req.query.word + '     story_oxford err:' + err
      );
      return next(err);
    }
  );
...
exports.story_oxford_ajax = function (req, res, next) {
  ...
  OxfordWord.find({ title: req.body.word, gubun: 'word' }).exec(function (
    err,
    theOxfordWord
  ) {
    if (err) {
    ...
    if (theOxfordWord.length > 0) {
      ...
      if (typeof theOxfordWord[0].kdata != 'undefined') {
        ...
        request(
          tooltip_naver_url + req.body.word,
          function (error, response, kdata) {
            if (error) {
              console.error(error);
            }
            console.log(`statusCode: ${response.statusCode}`);
            console.log(kdata);
            if (req.body.word.indexOf('_') < 0)
      ...
      var lookup = dict.find(props);
      console.log('자자 새롭게 해봅시다.');
      lookup.then(
        function (data) {
          console.log('여기 여기 parse result ->' + JSON.stringify(data));
          var results = data.results;
          var derivative_word = '';
          ...
          if (isDerivativeOf) {
            ...
            lookup4.then(
              function (data4) {
                ...
                request(
                  tooltip_naver_url + derivative_word,
                  function (error, response, kdata) {
                    if (error) {
                      console.error(error);
                    }
                        ...
                        request.post(options, function (er, rsp, body) {
                          if (!er && rsp.statusCode == 200) {
                            ...
                            if (translation.trim() == '') {
                              ...
                              if (translation.indexOf(ntranslation) < 0)
                                translation += ', ' + ntranslation;
                            }
                            ...
                            oxfordWord.save(function (err, theOxfordWord) {
                              ...
                              console.log(
                                'theOxfordWord._id 1:' + theOxfordWord._id
                              );
        ...
        function (err) {
          console.log(
            '헤이 storyController.js story_oxford_ajax req.query.word:' +
              req.body.word +
              ' story_oxford_ajax err:' +
              JSON.stringify(err)
          );
          if (JSON.stringify(err).indexOf('No such entry found.') > -1) {
            ...
            lookup2.then(
              function (data2) {
                ...
                var lemmas_word =
                  data2.results[0].lexicalEntries[0].inflectionOf[0].text;
                for (let i = 0; i < lexicalEntries.length; i++) {
                  console.log(
                    'lexicalEntries[i].inflectionOf[0].text ->' +
                      lexicalEntries[i].inflectionOf[0].text
                  );
                  if (req.body.word != lexicalEntries[i].inflectionOf[0].text) {
                    lemmas_word = lexicalEntries[
                      i
                    ].inflectionOf[0].text.replace(/-/g, '');
                    break;
                  }
                }
...
exports.story_oxford_ajaxt = function (req, res, next) {
  ...
  OxfordWord.find({ title: req.body.word, gubun: 'thesaurus' }).exec(function (
    err,
    theOxfordWord
  ) {
    if (err) {
...
exports.story_oxford_ajaxs = function (req, res, next) {
  ...
  OxfordWord.find({ title: req.body.word, gubun: 'sentences' }).exec(function (
    err,
    theOxfordWord
  ) {
    if (err) {
...
exports.story_word_datatable = function (req, res, next) {
  ...
  var pc = req.device.type.toUpperCase() == 'DESKTOP' ? 'DESKTOP' : '';
  res.render('story_word_list', {
    title: 'Story Word List',
    hostname: req.headers.host,
    pc: pc,
    cfnt: req.session.cfnt,
    cfnt2: req.session.cfnt2,
        ...
        .exec(function (err, list_stories) {
          ...
          for (let i = 0; i < list_stories.length; i++) {
            ...
            if (list_stories[i].create_date != null) {
              list_stories[i].create_date = moment(
                list_stories[i].create_date
              ).format('YYYY-MM-DD');
            }
            list_stories[i].title = entities.decodeHTML(list_stories[i].title);
            if (list_stories[i].btitle != null) {
              list_stories[i].btitle = entities.decodeHTML(
                list_stories[i].btitle
              );
            }
            var book;
            if (
              list_stories[i].book != null &&
              list_stories[i].book.title != null
            ) {
              book = {
                _id: list_stories[i].book._id,
                title: list_stories[i].book.title,
              };
            } else {
              book = {};
            }
...
exports.sendMessage = async (req, res, next) => {
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to send message to OpenAI. Status: ${response.status}`
      );
    }
