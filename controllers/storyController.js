...
exports.story_open_list = function (req, res, next) {
  ...
  if (typeof req.body.stle != "undefined" && req.body.stle != "") {
    Story.find({
      open: "Y",
      $or: [{ title: { $regex: ".*" + req.body.stle + ".*" } }, { btitle: { $regex: ".*" + req.body.stle + ".*" } }],
    })
      .count()
  ...
  async.parallel(
    {
      story: function (callback) {
        Story.findById(req.params.id).populate("genre").populate("book").exec(callback);
      },
      ...
      bookMark: function (callback) {
        BookMark.find({ user: req.session.userId, story: req.params.id }).exec(callback);
      },
      files: function (callback) {
        File.find({ user: req.session.userId, story: req.params.id }).exec(callback);
      },
      /*
        files: function(callback) {
      ...
      if (req.body.tlp == "y") {
        ...
        for (let i = 0; i < results.words.length; i++) {
          if (typeof results.words[i].image_address != "undefined" && typeof results.words[i].content != "undefined") {
            ...
            if (typeof results.words[i].add_content != "undefined") add_cnt = results.words[i].add_content;
            sTag +=
              '<div id="' + ttl + '" style="line-height: 1.2;">' + results.words[i].content + add_cnt + "</div>\n";
            highlightHtml = '<span class="tltp" data-tooltip-content="#' + ttl + '">$1</span>';
          } else {
  ...
  async.parallel(
    {
      ...
      genres: function (callback) {
        Genre.find({ user: req.session.userId }, "name ").sort({ name: 1 }).exec(callback);
      },
    },
    function (err, results) {
      if (err) {
...
exports.story_oxford = function (req, res, next) {
  ...
  lookup.then(
    ...
    function (err) {
      console.log("req.query.word:" + req.query.word + "     story_oxford err:" + err);
      return next(err);
    }
  );
};
      ...
      Story.find(searchStr)
        ...
        .exec(function (err, list_stories) {
          ...
          for (let i = 0; i < list_stories.length; i++) {
            ...
            if (typeof wrd === "undefined" || wrd.trim() === "") continue;
            var rwrd = wrd.toLowerCase();
            var gs = sword;
            var rgs = gs.toLowerCase();
            var idx = 0;
            for (let j = rwrd.indexOf(rgs); j >= 0; j = rwrd.indexOf(rgs, j + 1)) {
