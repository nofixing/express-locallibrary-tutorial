...
exports.story_list = function(req, res, next) {
  ...
  Story.find({$and:[{user: req.session.userId}, {book: null}] }).collation({locale: 'en' }).sort({create_date: -1})
    .exec(function (err, list_stories) {
      ...
      res.render('story_list', { title: 'Story List', story_list:  list_stories, pc: pc, cfnt: req.session.cfnt, cfnt2: req.session.cfnt2, cfwt: req.session.cfwt });
    });
...
exports.withdrawal = function(req, res, next) {
	...
	res.render('withdrawal', { hostname: req.headers.host, cfnt: req.session.cfnt, cfnt2: req.session.cfnt2, cfwt: req.session.cfwt });

};

exports.privacy_policy = function(req, res, next) {

	res.render('privacyPolicy', { cfnt: req.session.cfnt, cfnt2: req.session.cfnt2, cfwt: req.session.cfwt });
...
exports.story_open_list = function(req, res, next) {
    ...
    if(typeof req.body.stle !='undefined' && req.body.stle != '') {
        ...
        Story.find({open: 'Y', $or:[ {title: { $regex: '.*' + req.body.stle + '.*' }}, {btitle: { $regex: '.*' + req.body.stle + '.*' }}]})
            ...
            var pc = req.device.type.toUpperCase() == 'DESKTOP' ? 'DESKTOP':'';
            res.render('story_open_list', { title: 'Story List', story_list:  list_stories, hostname: req.headers.host, pc: pc, mxcnt: mxcnt+50, ct: ct, cfnt: req.session.cfnt, cfnt2: req.session.cfnt2, cfwt: req.session.cfwt });
        });
    } else {
...
exports.story_detail = function(req, res, next) {
    ...
    async.parallel({
        ...
        if(req.body.tlp == 'y') {
            ...
            for (let i = 0; i < results.words.length; i++) {
                if (typeof results.words[i].image_address != 'undefined' && typeof results.words[i].content != 'undefined') {
                    ...
                    sTag += '<div id="'+ttl+'" style="line-height: 1.2;">'+ssc+results.words[i].content+add_cnt+'</div>\n';
                    ...
                    var add_cnt = '';
                    if(typeof results.words[i].add_content != 'undefined') add_cnt = results.words[i].add_content;
                    sTag += '<div id="'+ttl+'" style="line-height: 1.2;">'+results.words[i].content+add_cnt+'</div>\n';
                    highlightHtml = '<span class="tltp" data-tooltip-content="#'+ttl+'">$1</span>';
                } else {
        ...
        */
        ...
        word_list:results.words, hostname: req.headers.host, pc: pc, userId: req.session.userId, cfnt: req.session.cfnt, cfnt2: req.session.cfnt2, cfwt: req.session.cfwt, cfnt2: req.session.cfnt2, book_title: book_title, book_id: book_id,
...
exports.story_create_get = function(req, res, next) {
    ...
    async.parallel({
        ...
        var pc = req.device.type.toUpperCase() == 'DESKTOP' ? 'DESKTOP':'';
        res.render('story_form', { title: 'Create Story',books:results.books,genres:results.genres, bok: bok, hostname: req.headers.host, pc: pc, cfnt: req.session.cfnt, cfnt2: req.session.cfnt2, cfwt: req.session.cfwt });
    });
...
exports.story_delete_get = function(req, res, next) {
    ...
    async.parallel({
        ...
        results.story.title = entities.decodeHTML(results.story.title);
        var pc = req.device.type.toUpperCase() == 'DESKTOP' ? 'DESKTOP':'';
        res.render('story_delete', { title: 'Delete Story', story: results.story, pc: pc, cfnt: req.session.cfnt, cfnt2: req.session.cfnt2, cfwt: req.session.cfwt } );
    });
...
exports.story_update_get = function(req, res, next) {
    ...
    async.parallel({
        ...
        results.story.title = entities.decodeHTML(results.story.title);
        var pc = req.device.type.toUpperCase() == 'DESKTOP' ? 'DESKTOP':'';
        res.render('story_form', { title: 'Update Story', books:results.books, genres:results.genres, story: results.story , hostname: req.headers.host, pc: pc, cfnt: req.session.cfnt, cfnt2: req.session.cfnt2, cfwt: req.session.cfwt, files:results.files });
    });
...
exports.story_preview = function(req, res, next) {
    ...
    res.render('preview', { content: req.body.content, cfnt: req.session.cfnt, cfnt2: req.session.cfnt2, cfwt: req.session.cfwt } );
...
exports.story_oxford_ajax = function(req, res, next) {
    ...
    OxfordWord.find({title: req.body.word, gubun: 'word'}).exec( function (err,theOxfordWord) {
        ...
        if (theOxfordWord.length > 0) {
            ...
            function(err) {
                console.log('헤이 storyController.js story_oxford_ajax req.query.word:'+req.body.word+' story_oxford_ajax err:'+JSON.stringify(err));
...
exports.story_word_datatable = function (req, res, next) {
    ...
    var pc = req.device.type.toUpperCase() == 'DESKTOP' ? 'DESKTOP':'';
    res.render('story_word_list', { title: 'Story Word List', hostname: req.headers.host, pc: pc, cfnt: req.session.cfnt, cfnt2: req.session.cfnt2, cfwt: req.session.cfwt, searchWord: searchWord });
};
