  ...
  async.parallel(
    {
      ...
      history: function (callback) {
        History.find({ user: req.session.userId })
          .populate({ path: 'story', select: 'chapter title' })
          .populate('book', 'title')
          .skip(0)
          .limit(5)
          .sort({ create_date: -1 })
    ...
    function (err, results) {
      ...
      if (req.session.userId) {
        ...
        for (let i = 0; i < results.history.length; i++) {
          results.history[i].story.title = entities.decodeHTML(
            results.history[i].story.title
          );
          if (results.history[i].book != null) {
            results.history[i].book.title = entities.decodeHTML(
              results.history[i].book.title
            );
          }
        }
        if (
          (clang != '' &&
            clang != 'undefined' &&
            typeof clang != 'undefined' &&
            clang == 'en') ||
          req.session.clang == 'en'
        ) {
          name += ',';
        } else {
          name += '님,';
        ...
        if (
          ...
          User.update(
            ...
            function (err, theUser) {
              ...
              req.session.cfnt = cfnt;
              console.log(
                'update cfnt:' + cfnt + '/update to:' + req.session.cfnt
              );
              res.render('index', {
                title: 'Welcome to Infinite Storlet',
                error: err,
                data: results,
                cert: cert,
                pc: pc,
          ...
          User.update(
            ...
            function (err, theUser) {
              ...
              req.session.cfwt = cfwt;
              console.log(
                'update cfwt:' + cfwt + '/update to:' + req.session.cfwt
              );
              res.render('index', {
                title: 'Welcome to Infinite Storlet',
                error: err,
  ...
  async.parallel(
    ...
    function (err, results) {
      ...
      for (let i = 0; i < results.book.genre.length; i++) {
        results.book.genre[i].name = entities.decodeHTML(
          results.book.genre[i].name
        );
      }
      for (let i = 0; i < results.stories.length; i++) {
        ...
        if (
          typeof results.stories[i].chapter == 'undefined' ||
...
exports.book_datatable_list = function (req, res, next) {
  ...
  if (typeof req.body.action == 'undefined') {
          ...
          .exec(function (err, list_books) {
            ...
            for (let i = 0; i < list_books.length; i++) {
              ...
              if (list_books[i].start_date != null) {
                list_books[i].start_date = moment(
                  list_books[i].start_date
                ).format('YYYY-MM-DD');
              }
              if (list_books[i].end_date != null) {
                list_books[i].end_date = moment(list_books[i].end_date).format(
                  'YYYY-MM-DD'
                );
              }
              if (list_books[i].create_date != null) {
                list_books[i].create_date = moment(
                  list_books[i].create_date
                ).format('YYYY-MM-DD');
              }
            }
    ...
    var sDt =
      obj[kyz[0]].start_date.trim() == ''
        ? ''
        : new Date(obj[kyz[0]].start_date);
    var eDt =
      obj[kyz[0]].end_date.trim() == '' ? '' : new Date(obj[kyz[0]].end_date);
    var cDt =
      obj[kyz[0]].create_date.trim() == ''
        ? ''
        : new Date(obj[kyz[0]].create_date);
    console.log('eDt[' + eDt + ']');
    Book.update(
      { _id: kyz },
      {
