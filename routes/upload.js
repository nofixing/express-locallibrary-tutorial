...
const multer = Multer({
  ...
  limits: {
    fileSize: 33 * 1024 * 1024, // no larger than 10mb, you can change as needed.
  },
});
...
router.post("/", multer.single("file_upload"), (req, res, next) => {
  ...
  const gcsFileName = `${req.session.userId}/${Date.now()}-${req.file.originalname}`;
  ...
  blobStream.on("finish", () => {
    ...
    file.save(function (err, theFile) {
      ...
      res.status(200).send(publicUrl + "&" + theFile._id + "&" + vsrc.substring(14));
    });
    /*
