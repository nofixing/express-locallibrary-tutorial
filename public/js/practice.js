...
$(function () {
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          if (data.result != data.fail) {
            ...
            var markup =
              "<tr style='min-height: 30px; vertical-align: top;'><td style='width: 120px; text-align: left;'>" +
              data.title +
              '</td>';
            markup +=
              "<td><input type='text' id='wrd_" +
              data.word_id +
            ...
            markup +=
              "<i class='fas fa-save' style='margin-left: 10px; cursor: pointer;' onclick='wrdSave(\"" +
              data.word_id +
              '")\'></i>';
            markup +=
              "<i class='fas fa-book-open' style='margin-left: 10px; cursor: pointer;' onclick='wordPopup(\"" +
              data.word_id +
              '")\'></i>';
            markup +=
              "<i class='fas fa-graduation-cap' style='margin-left: 10px; cursor: pointer;' onclick='oxfordPopup(\"" +
              data.title +
              '")\'></i>';
            markup +=
              "<i class='fas fa-photo-video' style='margin-left: 10px; cursor: pointer;' onclick='imgAddress(" +
              ldxx +
              ")'></i>";
            markup +=
              "<i class='fas fa-trash-alt' style='margin-left: 10px; cursor: pointer;' onclick='wordDelete(this, \"" +
              data.word_id +
              '")\'></i></span>';
            markup +=
              "<input type='hidden' class='wList' value='" +
              data.word_id +
              "'>";
            markup +=
              "<input type='hidden' class='ipt' value='" + data.title + "'>";
  ...
  $('#wordAdd').click(function () {
    ...
    var markup =
      "<tr><td style='text-align: center;'><div class='checkbox'><input type='checkbox' class='wList' value=''></div></td>";
    markup +=
      "<td><input type='text' size='15' maxlength='30' class='form-control ipt' value=''>";
    markup += "<input type='hidden' class='form-control iph' value=''>";

    markup +=
      "<select class='importance form-control' style='margin-top: 10px;'>";
    ...
    markup +=
      "<option value='4'>" + $('#Indispensable').val() + '</option></td>';

    markup +=
      "<td><textarea class='form-control txt' rows='1' cols='45'></textarea>";

    markup +=
      "<select class='skill form-control' style='display: inline; width: 200px; margin-top: 10px;'>";
    ...
    markup += '</select>';
    markup +=
      "<button type='button' class='btn btn-primary' style='display: inline; float: right; margin-top: 8px;' onclick='imgAddress(" +
      ldxx +
      ")'>";
      ...
      function (ev) {
        ...
        oReq.onload = function (oEvent) {
          if (oReq.status == 200) {
            ...
            var markup =
              "<tr><td><span id='" + arr[1] + "'>" + arr[2] + '</span>';
            markup +=
              "&nbsp;<button class='btn btn-primary' onclick=CopyFilePath('" +
              arr[0] +
              "')>" +
              $('#cptr').val() +
              '</button>';
...
function wordPopup(w_id) {
  ...
  if ($('#hostname').val().indexOf('localhost') > -1) httpType = 'http://';
  window.open(
    httpType + $('#hostname').val() + '/catalog/word_popup?w_id=' + w_id,
    '_blank',
    'toolbar=yes,scrollbars=yes,resizable=yes,top=250,left=200,width=650,height=600'
  );
}
...
function wordList() {
  ...
  /*
    ...
    */
  var selectedW =
    "<span class='ql-font-" + $('#cfnt').val() + "'>" + selectText + '</span>';
  var selectedC = selectContent;
  if ($('#cfnt2').val() != '')
    selectedC =
      "<span style='font-size: 1.5em;' class='ql-font-" +
      $('#cfnt2').val() +
      "'>" +
      selectedC +
      '</span>';
...
function wordList2(chatMessage) {
  var myEditor = document.querySelector('#snow-container');
  var html = myEditor.children[0].innerHTML;
  document.querySelector('#snow-container').children[0].innerHTML =
    html + chatMessage;
...
function saveChatGPT(saveChatMessage) {
  var data = {};
  data.story_id = $('#story_id').val();
  data.story_user = $('#stusr').val();
  ...
  $.ajax({
    ...
    data: JSON.stringify(data),
    contentType: 'application/json',
    url: httpType + $('#hostname').val() + '/catalog/chatGPT/create',
    success: function (data) {
      $('#chatGPT_id').val(data.chatGPT_id);
    },
  });
}
...
function preview() {
  ...
  if ($('#story_id').val() != '') {
    form.setAttribute(
      'action',
      '/catalog/story/' + $('#story_id').val() + '/preview'
    );
  } else {
...
function processDicData(dic_content, kdata, translation) {
  dic_content = dic_content.replace(/http:/g, 'https:');
  var simpleData = [];
  /*
  ...
  var cptxtb =
    "`);return false;' style='cursor: pointer;align:right;padding-right:30px;'><i class='fa'>&#xf0c5;</i></a>";
  var word_id = voca.id;
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (i == 0) {
      headWord =
        '<a onclick="sentence(\'' +
        word +
        '\');" style="color: hotpink; text-decoration: underline; cursor: pointer;">' +
        word +
        '</a>&nbsp;&nbsp;&nbsp;&nbsp;' +
        kword;
      headWord += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
      headWord +=
        '<a href="#" onclick="closeDic();return false;" style="cursor: pointer;align:right;padding-right:30px;">' +
        $('#close').val() +
        '</a>';
    } else {
      headWord =
    ...
    for (let j = 0; j < lexicalEntries.length; j++) {
      ...
      if (typeof lexicalCategory === 'object') {
        ...
        simpleData[i].nodes.push({
          text: lexicalCategory.text,
          selectable: true,
        });
      }
      ...
      if (typeof pronunciations === 'object') {
        for (let k = 0; k < pronunciations.length; k++) {
          ...
          if (typeof audioFile !== 'undefined') {
            ...
            simpleData[i].nodes[j].text =
              simpleData[i].nodes[j].text +
              '&nbsp;<a onclick="playAudio(this);" style="cursor: pointer;" class="fas fa-volume-up" src="' +
              audioFile +
              '"></a>';
          }
      ...
      if (typeof derivativeOf === 'object') {
        ...
        simpleData[i].nodes.push({
          text: 'derivativeOf: ' + derivativeOf[0].text,
          selectable: true,
        });
      }
      ...
      if (typeof derivatives === 'object') {
        ...
        simpleData[i].nodes.push({
          text: 'derivatives: ' + derivatives[0].text,
          selectable: true,
        });
      }
      ...
      for (let k = 0; k < entries.length; k++) {
        ...
        if (typeof etymologies === 'object') {
          ...
          //   console.log(
          //     'simpleData[' + i + ']: ' + JSON.stringify(simpleData[i])
          //   );
        ...
        if (typeof entries_pronunciations === 'object') {
          for (let l = 0; l < entries_pronunciations.length; l++) {
            ...
            if (typeof entries_audioFile !== 'undefined') {
              ...
              if (typeof simpleData[i].nodes[j].nodes !== 'object') {
                ...
                for (let v = 0; v < simpleData[i].nodes[j].nodes.length; v++) {
                  if (
                    simpleData[i].nodes[j].nodes[v].text.indexOf('mp3') > -1
                  ) {
                    isExists = true;
                    isIdx = v;
                  }
                ...
                if (isExists) {
                  simpleData[i].nodes[j].nodes[isIdx].text =
                    simpleData[i].nodes[j].nodes[isIdx].text +
                    '&nbsp;<a onclick="playAudio(this);" style="cursor: pointer;" class="fas fa-volume-up" src="' +
                    entries_audioFile +
                    '"></a>';
            ...
            var entries_phoneticNotation =
              entries_pronunciations[l].phoneticNotation;
            var entries_phoneticSpelling =
              entries_pronunciations[l].phoneticSpelling;
          }
        }
        ...
        for (let l = 0; l < senses.length; l++) {
            ...
            for (let m = 0; m < examples.length; m++) {
              ...
              simpleData[i].nodes[j].nodes.push({
                text:
                  ...
                  '</i>' +
                  cptxtf +
                  JSON.stringify(examples[m].text)
                    .replace(/\"/g, '')
                    .replace(/'/g, '‡') +
                  cptxtb,
                selectable: true,
          ...
          if (typeof shortDefinitions !== 'undefined') {
            ...
            simpleData[i].nodes[j].nodes.push({
              text:
                ...
                shortDefinitions +
                cptxtf +
                JSON.stringify(shortDefinitions[0])
                  .replace(/\"/g, '')
                  .replace(/'/g, '‡') +
                cptxtb,
              selectable: true,
            ...
            for (let m = 0; m < notes.length; m++) {
              simpleData[i].nodes[j].nodes.push({
                text:
                  ...
                  ')' +
                  cptxtf +
                  JSON.stringify(notes[m].text)
                    .replace(/\"/g, '')
                    .replace(/'/g, '‡') +
                  ' (' +
                  JSON.stringify(notes[m].type)
                    .replace(/\"/g, '')
                    .replace(/'/g, '‡') +
                  ')' +
                  cptxtb,
          ...
          if (typeof subsenses === 'object') {
            if (typeof simpleData[i].nodes[j].nodes[ssIdx].nodes !== 'object')
              simpleData[i].nodes[j].nodes[ssIdx].nodes = [];
            for (let m = 0; m < subsenses.length; m++) {
              ...
              if (typeof subsenses_definitions !== 'undefined') {
                ...
                simpleData[i].nodes[j].nodes[ssIdx].nodes.push({
                  text:
                    ...
                    '</b>' +
                    cptxtf +
                    JSON.stringify(subsenses_definitions[0])
                      .replace(/\"/g, '')
                      .replace(/'/g, '‡') +
                    cptxtb,
                  selectable: true,
              ...
              if (typeof subsenses_examples === 'object') {
                for (let n = 0; n < subsenses_examples.length; n++) {
                  var subsenses_examples_registers =
                    subsenses_examples[n].registers;
                  ...
                  simpleData[i].nodes[j].nodes[ssIdx].nodes.push({
                    text:
                      '<i>' +
                      subsenses_examples[n].text +
                      '</i>' +
                      cptxtf +
                      JSON.stringify(subsenses_examples[n].text)
                        .replace(/\"/g, '')
                        .replace(/'/g, '‡') +
                      cptxtb,
                    selectable: true,
          ...
          if (typeof thesaurusLinks === 'object') {
            simpleData[i].nodes[j].nodes.push({
              text:
                'thesaurus: <a onclick="thesaurus(\'' +
                thesaurusLinks[0].entry_id +
                '\');" style="color: hotpink; text-decoration: underline; cursor: pointer;">' +
                thesaurusLinks[0].entry_id +
                '</a>',
              selectable: true,
...
function processDicDatat(dic_content) {
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    if (i == 0) {
      headWord =
        '<a onclick="dicSearch();" style="color: hotpink; text-decoration: underline; cursor: pointer;">' +
        word +
        '</a>';
      headWord += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
      headWord +=
        ...
        for (let l = 0; l < senses.length; l++) {
          ...
          if (typeof antonyms === 'object') {
            for (let m = 0; m < antonyms.length; m++) {
              simpleData[i].nodes[j].nodes.push({
                text: 'antonym: ' + '<b>' + antonyms[m].text + '</b>',
              });
              ssIdx = simpleData[i].nodes[j].nodes.length - 1;
            }
          }
            ...
            for (let m = 0; m < subsenses.length; m++) {
              ...
              if (typeof subsenses_synonyms === 'object') {
                for (let n = 0; n < subsenses_synonyms.length; n++) {
                  simpleData[i].nodes[j].nodes[ssIdx].nodes.push({
                    text:
                      'synonym: ' + '<b>' + subsenses_synonyms[n].text + '</b>',
                  });
                }
              }
            }
          ...
          if (typeof synonyms === 'object') {
            for (let m = 0; m < synonyms.length; m++) {
              simpleData[i].nodes[j].nodes.push({
                text: 'synonym: ' + '<b>' + synonyms[m].text + '</b>',
              });
              ssIdx = simpleData[i].nodes[j].nodes.length - 1;
            }
          }
    ...
    for (let j = 0; j < lexicalEntries.length; j++) {
      ...
      for (let k = 0; k < sentences.length; k++) {
        ...
        if (typeof regions === 'object') {
          simpleData[i].nodes[j].nodes.push({
            text: regions[0].text + ': ' + '<b><i>' + text + '</i></b>',
          });
        } else {
          simpleData[i].nodes[j].nodes.push({
            text: '<b><i>' + text + '</i></b>',
